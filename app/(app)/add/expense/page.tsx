...
export default function AddExpensePage() {
  ...
  const { data: rawCats = [], isLoading: loadingCats } = useExpenseCategories();
  const { data: accounts = [] } = useAccounts();

  const categories = rawCats.map((c) => ({ id: c.expense_category_id, name: c.expense_category_name }));
    ...
    return (
      ...
      >
        <motion.div
          ...
          className="w-20 h-20 rounded-full bg-expense/15 border border-expense/30 flex items-center justify-center"
        >
          <CheckCircle2 size={40} className="text-expense" />
        </motion.div>
        <motion.div
        ...
        >
          <p className="text-white font-bold text-xl">Expense Added</p>
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="px-4 sm:px-8 pt-5 pb-3 shrink-0 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-white text-xl font-bold flex items-center gap-2">
            <TrendingDown size={20} className="text-expense" /> Expense
          </h1>
          <p className="text-pale text-xs mt-0.5">Record a new expense</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-8 pb-8 flex flex-col gap-4">
        ...
        <div className="bg-surface border border-expense/20 rounded-3xl px-5 py-5 flex flex-col items-center"
          style={{ background: "rgba(255,92,122,0.05)" }}>
          <p className="text-pale text-xs uppercase tracking-wide mb-3">Amount</p>
          <div className="flex items-center gap-1">
            <span className="text-expense/60 text-3xl font-bold">₹</span>
            <input
              type="number"
              inputMode="decimal"
              value={amount}
          ...
          <AnimatePresence>
            {num > 0 && (
              <motion.p
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -6 }}
                className="text-expense/70 text-xs mt-2 font-medium"
              >
        ...
        {/* Name */}
        <div>
          <label className="text-pale text-xs mb-2 block uppercase tracking-wide">What did you spend on?</label>
          <input
            type="text"
            value={name}
            onChange={(e) => setName(e.target.value)}
            placeholder="e.g. Lunch, Fuel, Netflix…"
            className="w-full bg-surface border border-border rounded-2xl px-4 py-3.5 text-white placeholder-muted text-sm focus:outline-none focus:border-expense transition-colors"
          />
        </div>

        {/* Category */}
        <div>
          <label className="text-pale text-xs mb-3 block uppercase tracking-wide px-0">Category</label>
          <CategoryPicker
        ...
        <AnimatePresence>
          {name.trim() && num > 0 && categoryId && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -8 }}
              className="bg-expense/10 border border-expense/20 rounded-2xl px-4 py-3 flex items-center justify-between"
            >
        ...
        {/* Slide to confirm */}
        <SlideToConfirm
          label={loading ? "Recording…" : "Slide to add expense"}
